...
WORKDIR /src
COPY ["Notifier.Lambda.csproj", "NotificationLambda/"]
RUN dotnet restore "NotificationLambda/Notifier.Lambda.csproj"

WORKDIR "/src/NotificationLambda"
COPY . .
RUN dotnet build "Notifier.Lambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Notifier.Lambda.csproj" \
...
Execute unit tests
```
cd "NotificationLambda/test/Notifier.Lambda.Tests"
dotnet test
```
